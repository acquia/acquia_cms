...
if [[ "$ACMS_JOB" == "base" ]] || [[ "$ACMS_JOB" == "starter" ]]; then
  ...
  orca fixture:init --force --sut=acquia/acquia_cms --sut-only --core=CURRENT --profile=acquia_cms --no-sqlite --no-site-install --project-template=acquia/drupal-recommended-project
