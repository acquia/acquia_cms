...
if [ -n "${ACMS_JOB}" ]; then
  ...
  echo '$settings["dx8_editable_api_url"] = TRUE;' >> ${ORCA_FIXTURE_DIR}/docroot/sites/default/default.settings.php
  echo '$config["cohesion.settings"]["api_url"] = "https://eu-development-api.sitestudio.acquia.com";' >> ${ORCA_FIXTURE_DIR}/docroot/sites/default/default.settings.php
  ...
  drush en acquia_cms_audio acquia_cms_dam sitestudio_config_management sitestudio_claro --yes --uri=http://127.0.0.1:8080
fi
