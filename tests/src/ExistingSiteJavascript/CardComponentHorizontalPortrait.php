...
/**
 * Tests that Horizontal (Portrait) Card Component.
 *
 * @group acquia_cms
 */
class CardComponentHorizontalPortrait extends CohesionTestBase {

  /**
   * Test that Horizontal (Portrait) card component is installed.
   *
  ...
  public function testComponentInstalled() {
    ...
    $page->fillField('Title', 'Cohesion card horizontal Portrait component');

    // Add the cohesion component in the field layout canvas.
    $canvas = $this->waitForElementVisible('css', '.coh-layout-canvas');
    $component_added = $this->addComponent($canvas, 'Card - Horizontal (Portrait)');
    ...
    $assert_session->pageTextContains('Page Cohesion card horizontal Portrait component has been created.');
  }
  ...
  public function testComponentUseEditAccess($role) {
    ...
    $assert_session->pageTextContains('Card - Horizontal (Portrait)');
    $this->getSession()->executeScript("jQuery('span:contains(\'Card - Horizontal (Portrait)\')').parents('tr:first').find('li.edit > a')[0].click()");
    ...
    $assert_session->pageTextContains('Saved the Component Card - Horizontal (Portrait).');
  }
