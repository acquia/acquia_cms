...
do
  ./acms-split.sh --branch=1.x --push=drupal --dry-run --module=${acms_module} && success=true || success=false
  exit_script ${success}
done

./acms-split.sh --branch=2.x --push=drupal --dry-run --module=acquia_cms_tour && success=true || success=false
exit_script ${success}
...
git push origin develop:1.x --dry-run
